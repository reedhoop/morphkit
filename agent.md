...
| `MorphTheme.kt` | `theme` | 语义色方法（`morphColorPrimary` 等）+ 形状 + 排版，全部走 `MaterialColors.getColor()` |
| `MorphTokens.kt` | `theme` | 静态设计 Token：iOS 颜色/形状/排版常量（camelCase 命名） |
...
| `BackdropBlurHelper.kt` | `widget.container` | `internal object`，`RenderEffect` GPU 高斯模糊 + 软件 Stack Blur 降级 |
...
*本文档最后更新：2026-06-10 | 基于质量报告全量修复 + minSdk 35 升级后更新*

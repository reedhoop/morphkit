...
│  MorphClickListener / InteractionMode            │
├─────────────────────────────────────────────────┤
│  皮肤层 (com.morphkit.theme)                     │
│  MorphTokens (嵌套 object 分层)                  │
│  MorphColors / MorphShape / MorphTypography      │
│  MorphStyleResolver / MorphTheme / MorphKitIOSConfig│
...
**Token 驱动设计：** 所有视觉属性通过设计 Token 统一管理。`MorphTokens` 按类别拆分为 7 个嵌套 object（Colors / Shapes / Typography / Interaction / Spacing / Elevation / Motion），外层通过委托属性保持 `MorphTokens.colorXxx` 的扁平化访问。View 体系通过 `MaterialColors.getColor(context, ...)` 从 Theme 读取，Compose 体系通过 `CompositionLocal` 传播，两套体系共享同一色彩真相源。

**线程安全：** 核心组件均采用线程安全设计——`MorphKit.config` 使用 `@Volatile`，`MorphInstaller.installed` 使用 `AtomicBoolean`，`MorphClickListener` 使用 `AtomicLong` + CAS 无锁防抖，`MorphStyleResolver.dynamicColorMethod` 使用 `lazy` 委托保证单次初始化，`MorphConfig` 内部使用 `ConcurrentHashMap`。

**内存优化：** `BackdropBlurHelper` 内置 Bitmap 对象池（最大 3 个）和 IntArray 双级缓存（主/次交替），滚动场景下大幅减少 GC 压力。`MorphCardView` 复用 `BitmapDrawable`（同尺寸直接替换像素），`MorphInitProvider` 注册 `onTrimMemory` 回调在内存压力时清空 Bitmap 池。
